...
export default async function handler(req, res) {
  try {
    ...
    if (perception) {
      ...
      return res.status(200).json({ perception, category });
    } else {
      return res.status(500).json({ error: "Failed to retrieve perception data" });
    }
  } catch (error) {
    console.error("Error occurred while processing perception:", error);
